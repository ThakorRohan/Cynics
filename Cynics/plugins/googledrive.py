...
from Cynics import ENV
from Cynics import gauth
from Cynics import GDRIVE_CREDENTIALS
from Cynics import setbot
from Cynics.languages.strings import tld
from Cynics.plugins.downloads import download_url
from Cynics.utils.parser import cleanhtml
...
async def get_drivedir(drive):
    ...
    for drivefolders in file_list:
        if drivefolders['title'] == 'Cynics Drive':
...
async def gdrive_stuff(client, message):
    gauth.LoadCredentialsFile('Cynics/session/drive')
    ...
    if len(
        ...
        try:
            os.rename(filename, 'Cynics/downloads/' + filename.replace(' ', '_'))
        except FileExistsError:
            os.rename(
                filename, 'Cynics/downloads/' + filename.replace(
                    ' ', '_',
                ) + '.2',
            )
        await edit_or_reply(
            ...
            text='Downloaded!\nFile saved to `{}`'.format(
                'Cynics/downloads/' + filename.replace(' ', '_'),
            ),
        )
    elif len(
        message.text.split(),
        ...
        filename = 'Cynics/downloads/{}'.format(filerealname.replace(' ', '_'))
        ...
        if message.reply_to_message:
            ...
            upload.SetContentFile('Cynics/downloads/' + nama)
            upload.Upload()
            upload.InsertPermission(
                {'type': 'anyone', 'value': 'anyone', 'role': 'reader'},
            )
            ...
            os.remove('Cynics/downloads/' + nama)
        else:
            await edit_or_reply(
                message,
