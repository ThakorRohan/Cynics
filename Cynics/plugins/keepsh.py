...
# any error / bugs please report to https://t.me/Cynicsbotsupport
# this module only support to Cynics-Bot userbot
import datetime
import os
...
async def tfsh(client, message):
    ...
    os.rename(
        fr'Cynics/downloads/{name}',
        fr'Cynics/downloads/{name_file_upload}',
    )
    await edit_or_reply(
        message,
        text=await send_to_keepsh(
            f'Cynics/downloads/{name_file_upload}',
            message, name_file_upload,
        ),
        disable_web_page_preview=True,
    )
    os.remove(f'Cynics/downloads/{name_file_upload}')
    return
