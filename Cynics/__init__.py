...
CYNICS_IMG = get_var('Cynics_IMG', False)
PM_PERMIT = bool(get_var('PM_PERMIT', False))

OwnerName = ''
app_version = f'💝 Cynics-Remix (v{USERBOT_VERSION})'
BotUsername = ''
BotID = 0
...
if os.path.exists('Cynics/logs/error.txt'):
    with open('Cynics/logs/error.txt', 'a') as f:
...
logging.basicConfig(
    ...
    format=LOG_FORMAT,
    datefmt='%m-%d %H:%M',
    filename='Cynics/logs/error.txt',
    filemode='w',
)
